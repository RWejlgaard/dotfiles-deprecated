...
chrome.runtime.onMessage.addListener(function (message, sender, response) {
	...
	if (window.location.href.match("https://www.turnoffthelights.com/extension/capture-screenshot-of-video.html")){
...
var visualvideos = document.getElementsByTagName("video");
	for(var i = 0; i < visualvideos.length; i++){
		...
		newonbutton.title = chrome.i18n.getMessage("titelvisenable");
		...
		newonchoosebutton.textContent = "❋ "+chrome.i18n.getMessage("titelvisblocks");
		newonchoosebutton.title = chrome.i18n.getMessage("titelvischoose");
		...
		newonlikebutton.title = chrome.i18n.getMessage("titelvideotoollike");
		...
		newonytsbutton.title = chrome.i18n.getMessage("titelvideotoolsubscribe");
		...
		newonrepeatbutton.addEventListener("click", function(){
			...
			if(onevideo){
				...
				if(onevideo.loop == true){onevideo.loop = false;this.title = chrome.i18n.getMessage("titelvideotoolrepeat") + " " + chrome.i18n.getMessage("titeloff");}else{onevideo.loop = true;this.title = chrome.i18n.getMessage("titelvideotoolrepeat") + " " + chrome.i18n.getMessage("titelon");}
			}
			}, false);
		...
		newonrepeatbutton.title = chrome.i18n.getMessage("titelvideotoolrepeat") + " " + chrome.i18n.getMessage("titeloff");
		...
		newonfilterbutton.addEventListener("click", function(){
			...
			if(onevideo){
				if(currentvideostepfilter == 0){
					...
					onevideo.style.webkitFilter = "grayscale(1)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolgrayscale");
				}
					...
					getstefanvdvideotoolrange.max = "1";
					getstefanvdvideotoolrange.value = "1";
					onevideo.style.webkitFilter = "sepia(1)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolsepia");
					}
					...
					getstefanvdvideotoolrange.max = "1";
					getstefanvdvideotoolrange.value = "1";
					onevideo.style.webkitFilter = "invert(1)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolinvert");
					}
					...
					onevideo.style.webkitFilter = "contrast(10)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolcontrast");
					}
					...
					getstefanvdvideotoolrange.max = "10";
					getstefanvdvideotoolrange.value = "10";
					onevideo.style.webkitFilter = "saturate(10)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolsaturate");
					}
					...
					onevideo.style.webkitFilter = "hue-rotate(90deg)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolhueroration");
					}
					...
					onevideo.style.webkitFilter = "brightness(1.5)";currentvideostepfilter +=1;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolbrightness");
					}
					...
					getstefanvdvideotoolrange.max = "10";
					getstefanvdvideotoolrange.value = "1";
					onevideo.style.webkitFilter = "";currentvideostepfilter = 0;newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolnormal");
					}
		...
		newonfilterbutton.title = chrome.i18n.getMessage("titelvideotoolfilter");
		...
		newvcpartiaspan.textContent = chrome.i18n.getMessage("titelvideotoolnormal");
		...
		newonfwbutton.title = chrome.i18n.getMessage("titelvideotoolfullwindow");
		...
		newscreenshotbutton.title = chrome.i18n.getMessage("titelvideotoolscreenshot");
...
function videovisualloop(tovideo){
		...
		requestvideovisualloop = window.requestAnimFrame(function(tempvis){videovisualloop(tempvis)});
		...
		var dolphin = tempvis;
		if(visualnumber == 1){
			document.getElementById('stefanvdvischoosebutton'+dolphin).textContent = "❋ "+chrome.i18n.getMessage("titelvisblocks");
			...
			document.getElementById('stefanvdvischoosebutton'+dolphin).textContent = "❋ "+chrome.i18n.getMessage("titelvisfrequency");
			ctx.clearRect(0, 0, w, h);
			analyser.getByteTimeDomainData(dataArray);
			...
			document.getElementById('stefanvdvischoosebutton'+dolphin).textContent = "❋ "+chrome.i18n.getMessage("titelvistunnel");
			ctx.clearRect(0,0,w,h);
			...
			var bctx1 = buffer1.getContext('2d', {desynchronized: true});
			var bctx2 = buffer2.getContext('2d', {desynchronized: true});
...
if(playrate == true){
	...
	for(var i = 0; i < ratevideos.length; i++){
		var myElement = document.getElementsByTagName("video")[i];
...
function gogonightmode(){
    ...
    if(sun == true){
			...
			if(window.location.href.match(/((http:\/\/(.*youtube\.com\/.*))|(https:\/\/(.*youtube\.com\/.*)))/i)){ }else{
				...
				document.body.className += " stefanvdnightbck stefanvdnight";
				}
				if(document.querySelector("html").classList.contains("stefanvdnightbck") || document.querySelector("html").classList.contains("stefanvdnight")){
				}else{
				document.querySelector("html").className += " stefanvdnightbck stefanvdnight";
				}
				...
				for (var i = 0; i < n.length; i++){
					...
					if(getthatid == "stefanvdnighttheme"){
						...
						if(startshow == true){
							...
							if(n[i].tagName == "IMG" || n[i].tagName == "STYLE" || n[i].tagName == "SCRIPT" || n[i].tagName == "HEAD" || n[i].tagName == "META" || n[i].tagName == "LINK" || n[i].tagName == "TITLE" || n[i].tagName == "IFRAME" || n[i].tagName == "svg" || n[i].tagName == "path" || n[i].tagName == "PICTURE" || n[i].tagName == "SOURCE" || n[i].tagName == "VIDEO" || n[i].tagName == "AUDIO" || n[i].tagName == "FIGURE" || n[i].tagName == "A"){}
							else{

								if(n[i].currentStyle){
									if(n[i].classList.contains("stefanvdnightbck") || n[i].classList.contains("stefanvdnight")){
									}else{
										var y = n[i].currentStyle["background-color"];
										var z = n[i].currentStyle["background-image"];
										...
										if(thatbckishere == true){
											n[i].className += " stefanvdnightbck stefanvdnight";
										}else{
									...
									if(n[i].classList.contains("stefanvdnightbck") || n[i].classList.contains("stefanvdnight")){
										...
										var st = document.defaultView.getComputedStyle(n[i], null);
			...
			for(var i = 0; i < ytytdapp.length; i++ ){ytytdapp[i].style.background = nightmodebck;}
			//var ytdotytdapp = document.querySelectorAll('.ytd-app');
			//for(var i = 0; i < ytdotytdapp.length; i++ ){ytdotytdapp[i].style.background = nightmodebck;}
			var ytironicon = document.querySelectorAll('iron-icon');
			for(var i = 0; i < ytironicon.length; i++ ){ytironicon[i].style.cssText = "-webkit-filter: grayscale(0%) brightness(0%) contrast(0%);";}
			...
			for(var i = 0; i < yth3ytd.length; i++ ){yth3ytd[i].style.color = nightmodetxt;}
			var ytbyline = document.querySelectorAll('#byline.ytd-video-meta-block');
			for(var i = 0; i < ytbyline.length; i++ ){ytbyline[i].style.color = nightmodetxt;}
			var ytmetadata = document.querySelectorAll('#metadata-line.ytd-video-meta-block');
			for(var i = 0; i < ytmetadata.length; i++ ){ytmetadata[i].style.color = nightmodetxt;}
			var ytironicon = document.querySelectorAll('.iron-icon-0');
			for(var i = 0; i < ytironicon.length; i++ ){ytironicon[i].style.color = nightmodetxt;}
			var ytverifiedbox = document.querySelectorAll('#guide-icon.ytd-topbar-logo-renderer');
			for(var i = 0; i < ytverifiedbox.length; i++ ){ytverifiedbox[i].style.fill = nightmodetxt;}
			var yttitlevideo = document.querySelectorAll('.ytd-video-primary-info-renderer');
			for(var i = 0; i < yttitlevideo.length; i++ ){yttitlevideo[i].style.color = nightmodetxt;}
			var ytforview = document.querySelectorAll('.yt-view-count-renderer');
			for(var i = 0; i < ytforview.length; i++ ){ytforview[i].style.color = nightmodetxt;}
			var ytsecondinfo = document.querySelectorAll('.ytd-video-secondary-info-renderer');
			for(var i = 0; i < ytsecondinfo.length; i++ ){ytsecondinfo[i].style.color = nightmodetxt;}
			var ytcommentshead = document.querySelectorAll('.ytd-comments-header-renderer');
			for(var i = 0; i < ytcommentshead.length; i++ ){ytcommentshead[i].style.color = nightmodetxt;}
			var ytcommentsimplebox = document.querySelectorAll('.ytd-comment-simplebox-renderer');
			for(var i = 0; i < ytcommentsimplebox.length; i++ ){ytcommentsimplebox[i].style.color = nightmodetxt;}
			var ytcommentrender = document.querySelectorAll('.ytd-comment-renderer');
			for(var i = 0; i < ytcommentrender.length; i++ ){ytcommentrender[i].style.color = nightmodetxt;}
			var ytcommentaction = document.querySelectorAll('.ytd-comment-action-buttons-renderer');
			for(var i = 0; i < ytcommentaction.length; i++ ){ytcommentaction[i].style.color = nightmodetxt;}
			var ytsigninpromo = document.querySelectorAll('.ytd-guide-signin-promo-renderer');
			for(var i = 0; i < ytsigninpromo.length; i++ ){ytsigninpromo[i].style.color = nightmodetxt;}
			var ytguidesectionrenderer = document.querySelectorAll('.ytd-guide-section-renderer');
			for(var i = 0; i < ytguidesectionrenderer.length; i++ ){ytguidesectionrenderer[i].style.color = nightmodetxt;}
			var ytguideentryhover = document.querySelectorAll('.ytd-guide-entry-renderer:hover');
			for(var i = 0; i < ytguideentryhover.length; i++ ){ytguideentryhover[i].style.backgroundColor = "#292929";}
			var ytgridrenderer = document.querySelectorAll('.ytd-grid-renderer');
			for(var i = 0; i < ytgridrenderer.length; i++ ){ytgridrenderer[i].style.color = nightmodetxt;}
			var ytaccountsettings = document.querySelectorAll('.ytd-account-settings-0');
			for(var i = 0; i < ytaccountsettings.length; i++ ){ytaccountsettings[i].style.backgroundColor = nightmodebck;}
			var ytmultipagerenderer = document.querySelectorAll('.ytd-multi-page-menu-renderer-0');
			for(var i = 0; i < ytmultipagerenderer.length; i++ ){ytmultipagerenderer[i].style.backgroundColor = nightmodebck;}
			var ytheadchannelname = document.querySelectorAll('.yt-endpoint-1');
			for(var i = 0; i < ytheadchannelname.length; i++ ){ytheadchannelname[i].style.color = nightmodetxt;}
			var ytendpoint = document.querySelectorAll('.yt-endpoint-3');
			for(var i = 0; i < ytendpoint.length; i++ ){ytendpoint[i].style.color = nightmodetxt;}
			var ytendpointfour = document.querySelectorAll('.yt-endpoint-4');
			for(var i = 0; i < ytendpointfour.length; i++ ){ytendpointfour[i].style.color = nightmodetxt;}
			var ytformatstring = document.querySelectorAll('yt-formatted-string');
			for(var i = 0; i < ytformatstring.length; i++ ){ytformatstring[i].style.color = nightmodetxt;}
			// update YouTube 27 December 2017
			var ytpagemanager = document.querySelectorAll('ytd-page-manager');
			for(var i = 0; i < ytpagemanager.length; i++){ytpagemanager[i].style.color = nightmodetxt;ytpagemanager[i].style.backgroundColor = nightmodebck;}
			var ytwatch = document.querySelectorAll('ytd-watch');
			for(var i = 0; i < ytwatch.length; i++){ytwatch[i].style.color = nightmodetxt;ytwatch[i].style.backgroundColor = nightmodebck;}
			var ytdtopbarlogorenderer = document.querySelectorAll('ytd-topbar-logo-renderer');
			for(var i = 0; i < ytdtopbarlogorenderer.length; i++){ytdtopbarlogorenderer[i].style.cssText = "-webkit-filter: invert(1) grayscale(1);";}
			...
			for(var i = 0; i < ytdguidenentry.length; i++){ytdguidenentry[i].style.color = nightmodetxt;ytdguidenentry[i].style.backgroundColor = nightmodebck;}
			var ytdguidenentry = document.querySelectorAll('.ytd-toggle-button-renderer');
			for(var i = 0; i < ytdguidenentry.length; i++){ytdguidenentry[i].style.color = nightmodetxt;}
			var ytdbuttonrenderer = document.querySelectorAll('yt-icon');
			for(var i = 0; i < ytdbuttonrenderer.length; i++){ytdbuttonrenderer[i].style.cssText = "-webkit-filter: invert(1) grayscale(1) contrast(0);";}
			var paperbutton = document.querySelectorAll('paper-button');
			for(var i = 0; i < paperbutton.length; i++){paperbutton[i].style.color = nightmodetxt;}
			var ytsimple = document.querySelectorAll('.yt-simple-endpoint style-scope ytd-guide-entry-renderer a');
			for(var i = 0; i < ytsimple.length; i++){ytsimple[i].style.color = nightmodetxt;}
			var ytcompact = document.querySelectorAll('.ytd-compact-autoplay-renderer');
			for(var i = 0; i < ytcompact.length; i++){ytcompact[i].style.color = nightmodetxt;}
			var ytgridvideorenderera = document.querySelectorAll('.style-scope ytd-grid-video-renderer a');
			for(var i = 0; i < ytgridvideorenderera.length; i++){ytgridvideorenderera[i].style.color = nightmodetxt;}
			var ytgridvideorendererspan = document.querySelectorAll('.style-scope ytd-grid-video-renderer span');
			for(var i = 0; i < ytgridvideorendererspan.length; i++){ytgridvideorendererspan[i].style.color = nightmodetxt;}
			var ytshelfspan = document.querySelectorAll('.style-scope ytd-shelf-renderer span');
			for(var i = 0; i < ytshelfspan.length; i++){ytshelfspan[i].style.color = nightmodetxt;}
			// update YouTube 8 March 2018
			var ytdbrowse = document.querySelectorAll('ytd-browse');
			for(var i = 0; i < ytdbrowse.length; i++){ytdbrowse[i].style.color = nightmodetxt;ytdbrowse[i].style.backgroundColor = nightmodebck;}
			...
			for(var i = 0; i < tabsinnerpapertabs.length; i++){tabsinnerpapertabs[i].style.color = nightmodetxt;}
			var ytdsearchboxcont = document.querySelectorAll('ytd-searchbox #container');
			for(var i = 0; i < ytdsearchboxcont.length; i++){ytdsearchboxcont[i].style.color = nightmodetxt;ytdsearchboxcont[i].style.backgroundColor = nightmodebck;}
			var h3ytdcompact = document.querySelectorAll('h3.ytd-compact-radio-renderer');
			for(var i = 0; i < h3ytdcompact.length; i++){h3ytdcompact[i].style.color = nightmodetxt;}
			// 27 June 2018
			var ytdbrowse = document.querySelectorAll('.ytd-watch-flexy');
			for(var i = 0; i < ytdbrowse.length; i++){ytdbrowse[i].style.color = nightmodetxt;ytdbrowse[i].style.backgroundColor = nightmodebck;}
			// 14 December 2018
			var papertabselected = document.querySelectorAll('.iron-selected');
			for(var i = 0; i < papertabselected.length; i++){papertabselected[i].style.color = nightmodetxt;}
			var searchboxtext = document.querySelectorAll('.ytd-searchbox');
			for(var i = 0; i < searchboxtext.length; i++){searchboxtext[i].style.color = nightmodetxt;}
			var titleytmini = document.querySelectorAll('.title.ytd-mini-channel-renderer');
			for(var i = 0; i < titleytmini.length; i++){titleytmini[i].style.color = nightmodetxt;}
			var ytverticalsection = document.querySelectorAll('.ytd-vertical-channel-section-renderer');
			for(var i = 0; i < ytverticalsection.length; i++){ytverticalsection[i].style.color = nightmodetxt;}
			var ytformattedstring = document.querySelectorAll('.yt-formatted-string');
			for(var i = 0; i < ytformattedstring.length; i++){ytformattedstring[i].style.color = nightmodetxt;}
...
//-----
        ...
        if (window.location.href.match(/((http:\/\/(.*youtube\.com\/.*))|(https:\/\/(.*youtube\.com\/.*)))/i)) {
			...
			// update new YouTube layout 19 december 2016
			var ytytdapp = document.querySelectorAll('ytd-app');
			for(var i = 0; i < ytytdapp.length; i++ ){ytytdapp[i].style.background = "white";}
			//var ytdotytdapp = document.querySelectorAll('.ytd-app');
			//for(var i = 0; i < ytdotytdapp.length; i++ ){ytdotytdapp[i].style.background = "white";}
			var ytironicon = document.querySelectorAll('iron-icon');
			for(var i = 0; i < ytironicon.length; i++ ){ytironicon[i].style.cssText = "";}
			if ($("masthead")) { $("masthead").style.background = "white"; }
			if ($("contents")) { $("contents").style.color = ""; }
			var yth3ytd = document.querySelectorAll('h3.ytd-compact-video-renderer');
			for(var i = 0; i < yth3ytd.length; i++ ){yth3ytd[i].style.color = "black";}
			var ytbyline = document.querySelectorAll('#byline.ytd-video-meta-block');
			for(var i = 0; i < ytbyline.length; i++ ){ytbyline[i].style.color = "black";}
			var ytmetadata = document.querySelectorAll('#metadata-line.ytd-video-meta-block');
			for(var i = 0; i < ytmetadata.length; i++ ){ytmetadata[i].style.color = "hsla(0, 0%, 6.7%, .6)";}
			var ytironicon = document.querySelectorAll('.iron-icon-0');
			for(var i = 0; i < ytironicon.length; i++ ){ytironicon[i].style.color = "hsla(0, 0%, 6.7%, .4)";;}
			var ytverifiedbox = document.querySelectorAll('#guide-icon.ytd-topbar-logo-renderer');
			for(var i = 0; i < ytverifiedbox.length; i++ ){ytverifiedbox[i].style.fill = "hsla(0, 0%, 6.7%, .4)";}
			var yttitlevideo = document.querySelectorAll('.ytd-video-primary-info-renderer');
			for(var i = 0; i < yttitlevideo.length; i++ ){yttitlevideo[i].style.color = "black";}
			var ytforview = document.querySelectorAll('.yt-view-count-renderer');
			for(var i = 0; i < ytforview.length; i++ ){ytforview[i].style.color = "hsla(0, 0%, 6.7%, .6)";}
			var ytsecondinfo = document.querySelectorAll('.ytd-video-secondary-info-renderer');
			for(var i = 0; i < ytsecondinfo.length; i++ ){ytsecondinfo[i].style.color = "";}
			var ytcommentshead = document.querySelectorAll('.ytd-comments-header-renderer');
			for(var i = 0; i < ytcommentshead.length; i++ ){ytcommentshead[i].style.color = "hsl(0, 0%, 6.7%)";}
			var ytcommentsimplebox = document.querySelectorAll('.ytd-comment-simplebox-renderer');
			for(var i = 0; i < ytcommentsimplebox.length; i++ ){ytcommentsimplebox[i].style.color = "hsla(0, 0%, 6.7%, .6)";}
			var ytcommentrender = document.querySelectorAll('.ytd-comment-renderer');
			for(var i = 0; i < ytcommentrender.length; i++ ){ytcommentrender[i].style.color = "hsl(0, 0%, 6.7%)";}
			var ytcommentaction = document.querySelectorAll('.ytd-comment-action-buttons-renderer');
			for(var i = 0; i < ytcommentaction.length; i++ ){ytcommentaction[i].style.color = "hsl(0, 0%, 6.7%)";}
			var ytsigninpromo = document.querySelectorAll('.ytd-guide-signin-promo-renderer');
			for(var i = 0; i < ytsigninpromo.length; i++ ){ytsigninpromo[i].style.color = "hsl(0, 0%, 6.7%)";}
			var ytguidesectionrenderer = document.querySelectorAll('.ytd-guide-section-renderer');
			for(var i = 0; i < ytguidesectionrenderer.length; i++ ){ytguidesectionrenderer[i].style.color = "hsla(0, 0%, 6.7%, .6)";}
			var ytguideentryhover = document.querySelectorAll('.ytd-guide-entry-renderer:hover');
			for(var i = 0; i < ytguideentryhover.length; i++ ){ytguideentryhover[i].style.backgroundColor = "hsl(0, 0%, 93.3%)";}
			var ytgridrenderer = document.querySelectorAll('.ytd-grid-renderer');
			for(var i = 0; i < ytgridrenderer.length; i++ ){ytgridrenderer[i].style.color = "hsla(0, 0%, 6.7%, .6)";}
			var ytaccountsettings = document.querySelectorAll('.ytd-account-settings-0');
			for(var i = 0; i < ytaccountsettings.length; i++ ){ytaccountsettings[i].style.backgroundColor = "hsl(0, 0%, 100%)";}
			var ytmultipagerenderer = document.querySelectorAll('.ytd-multi-page-menu-renderer-0');
			for(var i = 0; i < ytmultipagerenderer.length; i++ ){ytmultipagerenderer[i].style.backgroundColor = "hsl(0, 0%, 100%)";}
			var ytheadchannelname = document.querySelectorAll('.yt-endpoint-1');
			for(var i = 0; i < ytheadchannelname.length; i++ ){ytheadchannelname[i].style.color = "";}
			var ytendpoint = document.querySelectorAll('.yt-endpoint-3');
			for(var i = 0; i < ytendpoint.length; i++ ){ytendpoint[i].style.color = "";}
			var ytendpointfour = document.querySelectorAll('.yt-endpoint-4');
			for(var i = 0; i < ytendpointfour.length; i++ ){ytendpointfour[i].style.color = "";}
			var ytformatstring = document.querySelectorAll('yt-formatted-string');
			for(var i = 0; i < ytformatstring.length; i++ ){ytformatstring[i].style.color = "";}
			// update YouTube 3 August 2017
			var ytpagemanager = document.querySelectorAll('ytd-page-manager');
			for(var i = 0; i < ytpagemanager.length; i++ ){ytpagemanager[i].style.color = "black";ytpagemanager[i].style.backgroundColor = "white";}
			var ytwatch = document.querySelectorAll('ytd-watch');
			for(var i = 0; i < ytwatch.length; i++ ){ytwatch[i].style.color = "black";ytwatch[i].style.backgroundColor = "white";}
			var ytdtopbarlogorenderer = document.querySelectorAll('ytd-topbar-logo-renderer');
			for(var i = 0; i < ytdtopbarlogorenderer.length; i++ ){ytdtopbarlogorenderer[i].style.cssText = "";}
			...
			var ytdguidenentry = document.querySelectorAll('.ytd-guide-entry-renderer');
			for(var i = 0; i < ytdguidenentry.length; i++){ytdguidenentry[i].style.color = "black";ytdguidenentry[i].style.backgroundColor = "white";}
			var ytdguidenentry = document.querySelectorAll('.ytd-toggle-button-renderer');
			for(var i = 0; i < ytdguidenentry.length; i++){ytdguidenentry[i].style.color = "";}
			var ytdbuttonrenderer = document.querySelectorAll('yt-icon');
			for(var i = 0; i < ytdbuttonrenderer.length; i++){ytdbuttonrenderer[i].style.cssText = "";}
			var paperbutton = document.querySelectorAll('paper-button');
			for(var i = 0; i < paperbutton.length; i++){paperbutton[i].style.color = "";}
			var ytsimple = document.querySelectorAll('.yt-simple-endpoint style-scope ytd-guide-entry-renderer a');
			for(var i = 0; i < ytsimple.length; i++){ytsimple[i].style.color = "";}
			var ytcompact = document.querySelectorAll('.ytd-compact-autoplay-renderer');
			for(var i = 0; i < ytcompact.length; i++){ytcompact[i].style.color = "";}
			var ytgridvideorenderera = document.querySelectorAll('.style-scope ytd-grid-video-renderer a');
			for(var i = 0; i < ytgridvideorenderera.length; i++){ytgridvideorenderera[i].style.color = "";}
			var ytgridvideorendererspan = document.querySelectorAll('.style-scope ytd-grid-video-renderer span');
			for(var i = 0; i < ytgridvideorendererspan.length; i++){ytgridvideorendererspan[i].style.color = "";}
			var ytshelfspan = document.querySelectorAll('.style-scope ytd-shelf-renderer span');
			for(var i = 0; i < ytshelfspan.length; i++){ytshelfspan[i].style.color = "";}
			// update YouTube 8 March 2018
			var ytdbrowse = document.querySelectorAll('ytd-browse');
			for(var i = 0; i < ytdbrowse.length; i++){ytdbrowse[i].style.color = "";ytdbrowse[i].style.backgroundColor = "";}
			...
			var tabsinnerpapertabs = document.querySelectorAll('#tabs-inner-container paper-tabs');
			for(var i = 0; i < tabsinnerpapertabs.length; i++){tabsinnerpapertabs[i].style.color = "";}
			var ytdsearchboxcont = document.querySelectorAll('ytd-searchbox #container');
			for(var i = 0; i < ytdsearchboxcont.length; i++){ytdsearchboxcont[i].style.color = "";ytdsearchboxcont[i].style.backgroundColor = "";}
			var h3ytdcompact = document.querySelectorAll('h3.ytd-compact-radio-renderer');
			for(var i = 0; i < h3ytdcompact.length; i++){h3ytdcompact[i].style.color = "";}
			// 27 June 2018
			var ytdbrowse = document.querySelectorAll('.ytd-watch-flexy');
			for(var i = 0; i < ytdbrowse.length; i++){ytdbrowse[i].style.color = nightmodetxt;ytdbrowse[i].style.backgroundColor = "";}
			// 14 December 2018
			var papertabselected = document.querySelectorAll('.iron-selected');
			for(var i = 0; i < papertabselected.length; i++){papertabselected[i].style.color = "";}
			var searchboxtext = document.querySelectorAll('.ytd-searchbox');
			for(var i = 0; i < searchboxtext.length; i++){searchboxtext[i].style.color = "";}
			var titleytmini = document.querySelectorAll('.title.ytd-mini-channel-renderer');
			for(var i = 0; i < titleytmini.length; i++){titleytmini[i].style.color = "";}
			var ytverticalsection = document.querySelectorAll('.ytd-vertical-channel-section-renderer');
			for(var i = 0; i < ytverticalsection.length; i++){ytverticalsection[i].style.color = "";}
			var ytformattedstring = document.querySelectorAll('.yt-formatted-string');
			for(var i = 0; i < ytformattedstring.length; i++){ytformattedstring[i].style.color = "";}
...
var doscroll=function(e){
		...
		if(isMac == true){
			if(delta== -1 && this.volume > 0.00){this.volume-=videovolumesteps;
				this.volume=Math.round(this.volume * 100) / 100
				setyoutubevolumemeter(this.volume);
			}
			if(delta== 1 && this.volume <= 0.99){
				this.volume+=videovolumesteps;this.volume=Math.round(this.volume * 100) / 100
				setyoutubevolumemeter(this.volume);
			}
		}else{
			if(delta== -1 && this.volume <= 0.99){
...
for(var i = 0; i < volumevideos.length; i++){
	...
	if(videovolumeposa == true){
		...
		newmousewheelvideo.style.top = position.y+35+tempscrolltop+"px";
		newmousewheelvideo.style.left = position.x+25+tempscrollleft+"px";
	}else if(videovolumeposc == true){
		newmousewheelvideo.style.top = position.y+35+tempscrolltop+"px";
		newmousewheelvideo.style.left = position.x+myElement.offsetWidth-85+tempscrollleft+"px";
	}
	...
	if(videovolume == true){
		volumevideos[i].addEventListener("wheel", doscroll, {passive: false});
		volumevideos[i].addEventListener("DOMMouseScroll", doscroll, false);
	}
...
window.addEventListener('resize', function(){
	var volumevideos = document.getElementsByTagName("video");
	for(var i = 0; i < volumevideos.length; i++){
		volumevideos[i].removeEventListener('wheel', doscroll, {passive: false});
		volumevideos[i].removeEventListener('DOMMouseScroll', doscroll, false);				
	}
...
chrome.runtime.onMessage.addListener(function(request, sender, sendMessage) {
	...
	else if(request.action == "gorefreshlight") {
	...
	else if(request.action == "gorefresheyedata") {
		chrome.storage.sync.get(['eyea','eyen'], function(items){
			if(items['eyea']){eyea = items['eyea'];}
			else{eyea = items['eyea'];}
			if(items['eyen']){eyen = items['eyen'];}
